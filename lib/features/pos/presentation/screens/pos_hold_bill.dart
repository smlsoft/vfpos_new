...
class _PosHoldBillState extends State<PosHoldBill> with TickerProviderStateMixin {
  ...
  Widget holdBillContent() {
    ...
    return GridView.builder(
        gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: 200, childAspectRatio: 3 / 2, crossAxisSpacing: 10, mainAxisSpacing: 10),
  ...
  Widget holdButton(PosHoldProcessModel hold) {
    ...
    if (global.posHoldProcessResult[global.findPosHoldProcessResultIndex(hold.code)].logCount != 0) {
    ...
    return ElevatedButton(
      style: ElevatedButton.styleFrom(padding: const EdgeInsets.all(0), backgroundColor: (global.posHoldActiveCode == hold.code) ? global.posTheme.secondary : backgroundColor),
      onPressed: () async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text((widget.holdType == 1) ? global.language("pos_hold_bill") : global.language("pos_hold_table")),
