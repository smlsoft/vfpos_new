...
Future<void> sendToKitchen({required String orderId, required List<OrderTempDataModel> orderList}) async {
  ...
  for (var kitchenCode in kitchenCodeActiveList) {
    KitchenObjectBoxStruct kitchen = kitchenList[kitchenList.indexWhere((element) => element.code == kitchenCode)];
    for (var order in orderList) {
      if (kitchen.products.contains(order.barcode) == true) {
        var orderTempUpdate = box.query(OrderTempObjectBoxStruct_.orderGuid.equals(order.orderGuid)).build().findFirst();
        ...
        if (kitchen.products.contains(order.barcode) == true) {
          ...
          if (findBarcode != null) {
            ...
            if (order.remark.trim().isNotEmpty) {
              printer.addCommand(PosPrintBillCommandModel(mode: 2, posStyles: const PosStyles(bold: true), columns: [
                FormDesignColumnModel(font_size: 24, width: 1, text: "x หมายเหตุ x : ${order.remark}", text_align: PrintColumnAlign.left),
              ]));
            }
            if (order.optionSelected.isNotEmpty) {
              List<OrderProductOptionModel> options = jsonDecode(order.optionSelected).map<OrderProductOptionModel>((item) => OrderProductOptionModel.fromJson(item)).toList();
                  ...
                  if (choice.selected!) {
                    if (optionPrint == false) {
                      printer.addCommand(PosPrintBillCommandModel(mode: 2, posStyles: const PosStyles(bold: false), columns: [
                        FormDesignColumnModel(font_size: 24, width: 1, text: " * ${option.names[0].name}", text_align: PrintColumnAlign.left),
                      ]));
                      optionPrint = true;
                    }
                    printer.addCommand(PosPrintBillCommandModel(mode: 2, posStyles: const PosStyles(bold: false), columns: [
                      FormDesignColumnModel(font_size: 24, width: 1, text: "   - ${choice.names[0].name}", text_align: PrintColumnAlign.left),
                    ]));
                  }
                }
              }
