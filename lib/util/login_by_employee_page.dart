...
class _LoginByEmployeeState extends State<LoginByEmployeePage> {
  ...
  void initState() {
    ...
    final String apiUser = global.getApiUserName();
    if (apiUser == "DEMOPOS") {
      userController.text = 'DEMOPOS';
    ...
    return BlocListener<AuthenticationBloc, AuthenticationState>(
      ...
      child: SafeArea(
          child: Scaffold(
              ...
              appBar: AppBar(
                title: Text(
                    "${global.language("sign_in")} ${global.applicationName} ${global.getAppversion()}"),
                actions: [
                  IconButton(
                      onPressed: () async {
                        List<BillObjectBoxStruct> bills =
                            (global.billHelper.selectSyncIsFalse());
                        if (bills.isEmpty) {
                          await showDialog(
                              context: context,
                          ...
                          await showDialog(
                              ...
                              builder: (BuildContext context) {
                                return AlertDialog(
                                  ...
                                  content: const Text(
                                      'มีข้อมูลที่ยังไม่ได้ Sync จากฐานข้อมูลเดิม กรุณาทำรายการใหม่ภายหลัง'),
                                  actions: [
                                    TextButton(
                                      onPressed: () {
                                        Navigator.pop(context);
