...
void main() {
  ...
  test('should generate promptpay data for identity correctly', () {
    expect(PromptPay.generateQRData("1100599000373", amount: 123.45),
        "00020101021129370016A000000677010111021311005990003735802TH53037645406123.456304FE9D");
  });
  ...
  test('extract qrcode from qr data', () {
    ...
    final indexOfStartAccountType =
        indexOfStartApplicationData + applicationIDData.length;
    ...
    final accountLength = int.parse(qrData.substring(
        indexOfStartAccountLength, indexOfStartAccountLength + 2));

    expect(accountLength, 13);

    final indexOfStartAccountData = indexOfStartAccountLength + 2;
    final accountData = qrData.substring(
        indexOfStartAccountData, indexOfStartAccountData + accountLength);
  ...
  test('extract qrcode from qr data with function', () {
    final qrData =
        "00020101021129370016A000000677010111011300668123456785802TH53037645406123.4563043045";
    expect(PromptPay.getAccountNumberFromQRData(qrData),
        Option.of("0066812345678"));
  });
  ...
  test('test promptpay constructor', () {
    ...
    expect(promptPayData.transferring?.data,
        "0016A00000067701011101130066812345678");
  });
}
